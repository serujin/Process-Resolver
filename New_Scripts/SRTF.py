...
class SRTF(Algorithm):
    ...
    def update_cpu_queue_logic(self):
        ...
        if self.queue_processes_are_identical(self.cpu_queue):
                self.cpu_queue = self.get_ordered_queue(self.cpu_queue, True, True)
        else:
            self.cpu_queue = self.get_ordered_queue(self.cpu_queue, True, self.current_time == 0)
    ...
    def cpu_has_to_exit(self):
        if not self.cpu_is_empty() and not self.cpu_queue_is_empty():
            return self.cpu.remaining_duration > self.cpu_queue[0].duration
